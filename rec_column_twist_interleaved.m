...
%--------------------------------------------------------------------------
function rec_column_twist_deinterleaved= rec_column_twist_interleaved(y)
  global nldpc_length
  global Nc
  global tc
  global mod_type
  ...
  if mod_type == 4
      rec_column_twist_deinterleaved = y;
      ...
      for i=1:column
          for k=1:row
               rec_column_twist_deinterleaved(column*(k-1) + 1 + mod(i + column-tc(k),column))= y((i-1)*row+k);
          end;
      end;
